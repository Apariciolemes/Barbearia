    ...
    <Sidebar :visible="visibleLeft" :baseZIndex="1000">
      <i class="pi pi-times close-x" @click="CloseAside"></i>
          <ul class="menu-wrapper m-0 p-2">
            ...
            <li class="menu-li px-1" @click="Render('Calendar')">
              <font-awesome-icon class="mr-3 icons" icon="calendar-alt" @click="Render('Calendar')"/>
              <a>Agenda</a>
            </li>
...
export default {
  ...
  data() {
    ...
    Render(item) {
      this.$store.state.renderComponent = item;
      this.visibleLeft = false;
    },
  },
};
